...
export function AvatarChip({
  ...
  const imageSrc = src ? resolveUploadUrl(src, cacheBust) : undefined;
  if (imageSrc) {
    return <img src={imageSrc} alt={name} className={`${sz} rounded-full object-cover shrink-0 border border-border`} />;
  }
  return (
