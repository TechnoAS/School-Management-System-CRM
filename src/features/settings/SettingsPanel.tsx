...
import { GraduationCap, Upload, Trash2, CheckCircle2, BadgeCheck, UserCog } from "lucide-react";
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          <Card className="p-5 flex flex-col items-center text-center">
            <div className="w-20 h-20 rounded-2xl bg-primary flex items-center justify-center mb-3 overflow-hidden">
              {settings.logoUrl ? (
                <img src={settings.logoUrl} alt="Institute logo" className="w-full h-full object-cover" />
              ) : (
                <GraduationCap size={32} className="text-white" />
              )}
            </div>
            <p className="text-sm font-semibold text-foreground mb-1">Institute Logo</p>
            <p className="text-xs text-muted-foreground mb-3">PNG, JPG or SVG, max 2MB</p>
            <label className="flex items-center gap-2 px-3 py-2 border border-dashed border-border rounded-lg cursor-pointer hover:bg-muted/30 text-xs text-muted-foreground transition-colors">
              <Upload size={12} /> {settings.logoUrl ? "Change logo" : "Upload new logo"}
