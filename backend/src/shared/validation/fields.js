import { z } from 'zod';
/**
 * Shared field patterns — keep in sync with `src/lib/validation/fields.ts` (front-end).
 */
...
export const v = {
    ...
    optionalPersonName: () => z.preprocess(emptyToNull, z
        .string()
        .trim()
        .regex(PATTERNS.personName, 'Name must be 2–120 letters (spaces, dots, hyphens allowed)')
        .nullable()
        .optional()),
    /** Accepts number OR numeric string (MySQL returns DECIMAL as string) */
    positiveMoney: () => z.preprocess((val) => (val === '' || val === undefined || val === null ? 0 : Number(val)), z.number().nonnegative('Amount cannot be negative')),
    gradeRequired: () => z
        .string()
        .regex(PATTERNS.grade, 'Grade must be A+, A, B+, B, C, or F')
        .refine((g) => g !== '-', 'Grade is required'),
    email: () => z.string().trim().email('Enter a valid email address').max(255),
    optionalEmail: () => z.preprocess(emptyToNull, z.string().trim().email('Enter a valid email address').max(255).nullable().optional()),
    phoneRequired: () => z
        .string()
        .trim()
        .regex(PATTERNS.phoneIndia, 'Phone must be a 10-digit mobile number starting with 6–9'),
    phoneOptional: () => z.preprocess(emptyToNull, z
        .string()
        .trim()
    ...
    password: () => z
        .string()
        .regex(PATTERNS.password, 'Password must be 8+ characters and include at least one letter and one number'),
    loginPassword: () => z.string().min(1, 'Password is required'),
    duration: () => z.string().trim().regex(PATTERNS.duration, 'Duration format: e.g. 6 Months, 12 Weeks'),
    experience: () => z.preprocess(emptyToNull, z
        .string()
        .trim()
        .regex(PATTERNS.experience, 'Experience format: e.g. 5 Years or Fresher')
        .nullable()
        .optional()),
    courseTitle: () => z.string().trim().regex(PATTERNS.title, 'Course name must be 2–255 characters'),
    examTitle: () => z.string().trim().regex(PATTERNS.title, 'Exam title must be 2–255 characters'),
    batchName: () => z.string().trim().regex(PATTERNS.batchName, 'Batch name must be 2–120 characters'),
    timing: () => z.string().trim().regex(PATTERNS.timing, 'Timing must be at least 3 characters'),
    address: () => z.string().trim().regex(PATTERNS.address, 'Address must be at least 5 characters'),
    addressOptional: () => z.preprocess(emptyToNull, z.string().trim().regex(PATTERNS.address, 'Address must be at least 5 characters').nullable().optional()),
    gradeOptional: () => z.preprocess(emptyToNull, z.string().regex(PATTERNS.grade, 'Grade must be A+, A, B+, B, C, F, or -').nullable().optional()),
    paymentMode: () => z
        .string()
        .regex(PATTERNS.paymentMode, 'Payment mode must be Cash, UPI, Card, Bank Transfer, Cheque, or Online'),
    receiptPrefix: () => z
        .string()
        .trim()
        .regex(PATTERNS.receiptPrefix, 'Prefix must be 1–20 letters, numbers, or hyphens'),
    receiptNumber: () => z.string().trim().regex(PATTERNS.receiptNumber, 'Starting number must be 1–10 digits'),
    academicYear: () => z.string().regex(PATTERNS.academicYear, 'Academic year format: 2024-25'),
    registrationNo: () => z.preprocess(emptyToNull, z
        .string()
        .trim()
        .regex(PATTERNS.registrationNo, 'Registration number format is invalid')
        .nullable()
        .optional()),
    qualification: () => z.preprocess(emptyToNull, z
        .string()
        .trim()
    ...
    subject: () => z
        .string()
        .trim()
        .regex(PATTERNS.title, 'Subject must be 2–120 characters'),
